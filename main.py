...
config = toml.load("/var/ossec/etc/config.toml")
...
if exists(config['options']['alertspath']):  # Only do if logs exist
    with open(config['options']['alertspath'],
        ...
        for logline in logfilejson:
            ...
            if not jsonlog['rule']['sidid'] in ruleids:
                if jsonlog['rule']['level'] >= alertlevel:
                    ...
                    logdetails = []
                    if not re.search(r"'decoder':", str(jsonlog)):  #if the logline has no decoder, it has no extra fields
                        decoder = "Unknown"
                    else:
                    ...
                    if condition: #insertion of uniqe logline
                        ...
                        if fullloginlog:  # if full log line is appended wie use levenshtein to determine the equality.
                            ...
                            index = [i for (i, log) in enumerate(logs) if (ratio(pattern_details, str(re.sub(r', Decoder: .*',"",str(re.sub(r'^Time(.*?), ', "", log))))) >= 0.92)]
                            if index:
